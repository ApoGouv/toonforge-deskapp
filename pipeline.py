...
class CartoonPipeline:
    ...
    def run_on_image(self, image):
        # Temporary pipeline for preview
        temp = CartoonPipeline(image)
        temp.smooth_passes = self.smooth_passes
        temp.num_colors = self.num_colors
        temp.edge_block = self.edge_block
        temp.edge_C = self.edge_C
        temp.smooth_mode = self.smooth_mode
        temp.color_mode = self.color_mode
        temp.blend_mode = self.blend_mode
        temp.use_denoise = self.use_denoise
